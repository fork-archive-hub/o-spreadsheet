...
export class SplitIntoColumnsPanel extends Component<Props, SpreadsheetChildEnv> {
  ...
  confirm() {
    interactiveSplitToColumns(this.env, this.separatorValue, this.state.addNewColumns);
  }
  ...
  get errorMessages(): string[] {
    ...
    for (const reason of cancelledReasons) {
      if (reason === CommandResult.SplitWillOverwriteContent) continue;
      if (reason === CommandResult.EmptySplitSeparator) continue;
  ...
  get warningMessages(): string[] {
    ...
    if (
      this.state.separatorValue !== "auto" &&
      !this.env.model.getters.isSeparatorInSelection(this.separatorValue)
    ) {
      warnings.push(SplitToColumnsTerms.Errors.NoSeparatorMatch);
    }
