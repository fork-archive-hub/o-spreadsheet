...
export function parseDateTime(str: string, locale: Locale): InternalDate | null {
  ...
  if (dateParts) {
    const separator = dateParts.str.match(dateSeparatorsRegex)![0];
    date = parseDate(dateParts, separator);
    if (date === null) {
      return null;
    }
    str = str.replace(dateParts.str, "").trim();
  }
 ...
 */
function getDateParts(str: string, locale: Locale): DateParts | null {
  ...
  if (part1.length > 2) {
    return { year: part1, month: part2, day: part3, str, type: "ymd" };
  }

  const localeDateType = getLocaleDateFormatType(locale);
  if (!part3) {
    if (localeDateType === "dmy") {
      return { day: part1, month: part2, year: part3, str, type: "dmy" };
    }
    return { month: part1, day: part2, year: part3, str, type: "mdy" };
  }
...
function parseDate(parts: DateParts, sep: string): InternalDate | null {
  ...
  const format = getFormatFromDateParts(parts, sep, leadingZero, fullYear);
