...
export class SplitToColumnsPlugin extends UIPlugin {
  static getters = ["getAutomaticSeparator", "isSeparatorInSelection"] as const;

  allowDispatch(cmd: Command) {
    switch (cmd.type) {
      case "SPLIT_TEXT_INTO_COLUMNS":
        return this.chainValidations(
          () => this.checkValidations(cmd, this.checkSingleColSelected, this.checkNonEmptySelector),
          this.checkNotOverwritingContent
        )(cmd);
    }
  ...
  isSeparatorInSelection(separator: string) {
    const cells = this.getters.getSelectedCells();
    for (const cell of cells) {
  ...
  private splitIntoColumns({ separator, addNewColumns }: SplitTextIntoColumnsCommand) {
    const fullSelection = this.getters.getSelectedZones();
    const sheetId = this.getters.getActiveSheetId();
    // Should never happen because of the allowDispatch, but just making sure
    if (fullSelection.length === 0 || fullSelection[0].left !== fullSelection[0].right) {
      throw new Error("More than one column selected");
    }
    const selection = fullSelection[0];
  ...
  private getSplittedCols(zone: Zone, separator: string): string[][] {
    if (!separator) throw new Error("Separator cannot be empty");
    const sheetId = this.getters.getActiveSheetId();
    const splitted: string[][] = [];
    for (const row of range(zone.top, zone.bottom + 1)) {
      const text = this.getters.getEvaluatedCell({ sheetId, col: zone.left, row }).formattedValue;
  ...
  private addColsToAvoidCollisions(zone: Zone, splittedCols: string[][]) {
    const sheetId = this.getters.getActiveSheetId();

    let colsToAdd = 0;

    for (const row of range(zone.top, zone.bottom + 1)) {
      const cellPosition = { sheetId, col: zone.left, row };
      const splittedText = splittedCols[row - zone.top];
  ...
  private addColumnsToNotOverflowSheet(selection: Zone, splittedCols: string[][]) {
    ...
    const maxColIndex = this.getters.getSheetSize(sheetId).numberOfCols - 1;
