...
function getGroup(
  ...
  for (let x of cells) {
    ...
    const cellValue = evaluateLiteral(x?.content, undefined, DEFAULT_LOCALE);
    if (filter(cellValue)) {
...
autofillRulesRegistry
  ...
  .add("increment_alphanumeric_value", {
    condition: (cell: Cell) =>
      !cell.isFormula &&
      evaluateLiteral(cell.content, undefined, DEFAULT_LOCALE).type === CellValueType.text &&
  ...
  .add("increment_number", {
    condition: (cell: Cell) =>
      !cell.isFormula &&
      evaluateLiteral(cell.content, undefined, DEFAULT_LOCALE).type === CellValueType.number,
    generateRule: (cell: Cell, cells: (Cell | undefined)[]) => {
      ...
      const increment = calculateIncrementBasedOnGroup(group);
      const evaluation = evaluateLiteral(cell.content, undefined, DEFAULT_LOCALE);
      return {
